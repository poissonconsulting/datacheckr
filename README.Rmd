...
There are several existing R packages for checking data including
...
The following `data_checkr` code states that `data1` 
...
(please file an issue if the code below can be improved)
```{r, results='hide'}
...
To repeat the above checks on several data frames can be 
achieved very efficiently using `check_data`
```{r, results='hide'}
data3 <- data2 <- data1
...
The `datacheckr` package exports a single function `data_check`
which takes two arguments: the data frame to check and a named list specifying
...
```
To specify that x should *not* contain a column called `mpg`
the call is just
```{r, error = TRUE}
...
as a vector of three or more missing values
```{r, results='hide'}
check_data(data1, list("Count" = c(0L, 1L, 3L)))
```
